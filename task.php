...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if (empty($project_id) || empty($title) || empty($date) || empty($time)) {
    $error_message = "Please fill in the required fields: Project id, title, date, time.";
            ...
            || strlen($datematch[1]) != 2 || strlen($datematch[2]) != 4
            || (!checkdate($datematch[1], $datematch[0], $datematch[2])) ) {
    echo var_dump($datematch);
                ...
                <table>
                    ...
                    <tr>
                        ...
                        <td><input type="text" id="title" name="title" value="
                          <?php echo htmlspecialchars($title); ?>" /></td>
                    </tr>
                    <tr>
                        <th><label for="date">Date<span class="required"></span></label></th>
                        <td><input type="text" id="date" name="date" value="
                          <?php echo htmlspecialchars($date); ?>" placeholder="mm/dd/yyyy" /></td>
                    </tr>
                    <tr>
                        <th><label for="time">Time<span class="required"></span></label></th>
                        <td><input type="text" id="time" name="time" value="
                          <?php echo htmlspecialchars($time); ?>" /> minutes</td>
                    </tr>
                </table>
                <?php
                if (!empty($task_id)) {
